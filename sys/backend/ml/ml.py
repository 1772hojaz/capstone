...
# Priority:
# 1. ML_MODEL_DIR environment variable (absolute or relative)
# 2. ./backend/ml_models when running from project root
# 3. ./ml_models when running from backend working dir
# 4. fallback to a ml_models folder next to this file
env_model_dir = os.environ.get("ML_MODEL_DIR")
if env_model_dir:
    # Allow users to override where models are stored (can be absolute or relative)
    MODEL_DIR = os.path.abspath(env_model_dir)
else:
    cwd_backend_ml = os.path.join(os.getcwd(), "backend", "ml_models")
    cwd_ml = os.path.join(os.getcwd(), "ml_models")
    if os.path.exists(cwd_backend_ml):
        MODEL_DIR = os.path.abspath(cwd_backend_ml)
    elif os.path.exists(cwd_ml):
        MODEL_DIR = os.path.abspath(cwd_ml)
    else:
