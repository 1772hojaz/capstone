...
class ColdStartHandler:
    ...
    def calculate_category_score(
        ...
        for i, cat_info in enumerate(preferred_categories):
            ...
            if category == product.category:
                rank = i + 1
                reason = f"Matches your #{rank} category ({category})"
                # Higher score for top categories
                category_score = 0.3 * score if isinstance(cat_info, dict) else 0.3 / rank
    ...
    def calculate_price_similarity(
        ...
        if min_price <= product_price <= max_price:
            ...
            if avg_price > 0:
                ...
                if price_score > 0.15:
                    reason = f"Matches your typical price range (${avg_price:.2f})"
                elif price_score > 0.05:
                    reason = "Within your price comfort zone"
                else:
                    reason = ""
            ...
            # Good deal!
            return 0.15, "Great price for you"
    ...
    def calculate_admin_group_cold_start_score(
        ...
        if admin_group.category:
            ...
            for i, cat_info in enumerate(preferred_categories):
                ...
                if category == admin_group.category:
                    rank = i + 1
                    category_reason = f"Matches your #{rank} category ({category})"
                    category_score = 0.3 * score if isinstance(cat_info, dict) else 0.3 / rank
                    break
        ...
        if admin_group.price:
            ...
            if min_price <= admin_group.price <= max_price:
                if avg_price > 0:
                    ...
                    if price_score > 0.15:
                        price_reason = f"Matches your typical price range (${avg_price:.2f})"
                    elif price_score > 0.05:
                        price_reason = "Within your price comfort zone"
            elif admin_group.price < min_price:
                price_score = 0.15
                price_reason = "Great price for you"
        ...
        if admin_group.max_participants and admin_group.participants:
            ...
            if moq_progress >= 75:
                dynamics_score += 0.1
                dynamics_reasons.append("Almost at target")
            elif moq_progress >= 50:
                dynamics_score += 0.05
                dynamics_reasons.append("Good progress")
        ...
        if admin_group.end_date:
            ...
            if days_remaining <= 3:
                dynamics_score += 0.05
                dynamics_reasons.append("Ending soon")
            elif days_remaining <= 7:
                dynamics_reasons.append("Limited time")
        
        # Savings
        if admin_group.discount_percentage and admin_group.discount_percentage >= 20:
            dynamics_score += 0.1
            dynamics_reasons.append(f"{admin_group.discount_percentage}% savings")
        elif admin_group.discount_percentage and admin_group.discount_percentage >= 10:
            dynamics_score += 0.05
